    ...
    <div className={`bg-[rgba(15,22,40,0.6)] rounded-xl overflow-hidden border transition-all duration-300 hover:shadow-[0_20px_40px_-12px_rgba(0,0,0,0.4)] hover:-translate-y-1 group ${isCompareSelected ? "border-[#4a90d9] ring-1 ring-[#4a90d9]/30" : "border-[rgba(74,144,217,0.12)] hover:border-[rgba(74,144,217,0.35)]"}`}>
      ...
      <div className="relative h-40 w-full bg-gradient-to-br from-[rgba(74,144,217,0.15)] to-[rgba(15,22,40,0.8)]">
        ...
        <div className="absolute top-3 left-3 right-3 flex items-center justify-between">
          ...
          {onCompareToggle && (
            <button
              onClick={(e) => { e.stopPropagation(); e.preventDefault(); onCompareToggle(provider); }}
              disabled={compareDisabled && !isCompareSelected}
              className={`flex items-center gap-1 px-2 py-1 text-[10px] font-medium rounded-md backdrop-blur-sm border transition-all duration-200 ${
                isCompareSelected
                  ? "bg-[#4a90d9] text-[#0a0f1a] border-[#4a90d9]"
                  : compareDisabled
                  ? "bg-[rgba(15,22,40,0.6)] text-[#3d4a61] border-[rgba(74,144,217,0.1)] cursor-not-allowed"
                  : "bg-[rgba(15,22,40,0.6)] text-[#e8edf5] border-[rgba(232,237,245,0.2)] hover:border-[#4a90d9] hover:text-[#4a90d9]"
              }`}
            >
      ...
      <div className="px-5 pb-5 pt-4">
        ...
        {/* Address */}
        <p className="text-[12px] text-[#6b7a94] truncate mb-3">{provider.address}</p>
