...
function LocationServiceContent({
  ...
  useEffect(() => {
    ...
    fetchProviders();
  }, [latitude, longitude, sortBy, searchQuery, service.searchQuery]);
  ...
  return (
    <div className="min-h-screen bg-[#0a0f1a] text-[#e8edf5]">
      ...
      {getServiceFAQs(service.slug).length > 0 && (
        <StructuredData data={generateFAQSchema(getServiceFAQs(service.slug))} />
      )}
      ...
      {(() => {
        const citySlug = `${cityName.toLowerCase().replace(/\s+/g, "-")}-${stateCode.toLowerCase()}`;
        const cityContent = getCityContent(citySlug, service.slug);
        if (!cityContent) return null;
        return (
          <section className="px-6 md:px-12 pb-8">
            <div className="max-w-[1400px] mx-auto">
              <div className="bg-[rgba(15,22,40,0.4)] border border-[rgba(74,144,217,0.1)] rounded-xl p-5 md:p-6">
                <p className="text-[14px] text-[#6b7a94] leading-relaxed mb-3">
                  {cityContent.intro}
                </p>
                <p className="text-[13px] text-[#4a90d9] flex items-start gap-2">
          ...
          {isLoading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {[...Array(6)].map((_, i) => (
                <div
                  key={i}
                  className="bg-[rgba(15,22,40,0.6)] rounded-xl overflow-hidden border border-[rgba(74,144,217,0.12)] animate-pulse"
                >
                  <div className="h-40 bg-[rgba(74,144,217,0.1)]" />
                  <div className="p-5">
                    <div className="h-4 bg-[rgba(74,144,217,0.1)] rounded mb-2 w-3/4" />
                    <div className="h-3 bg-[rgba(74,144,217,0.1)] rounded w-1/2 mb-3" />
            ...
            <div className="text-center py-12">
              <div className="w-12 h-12 rounded-full bg-[rgba(74,144,217,0.1)] flex items-center justify-center mx-auto mb-4">
              ...
              {activeFilters.length > 0 && (
                <div className="space-y-3">
                  ...
                  <button
                    onClick={() => setActiveFilters([])}
                    className="px-5 py-2 text-[12px] font-medium bg-[#4a90d9] text-[#0a0f1a] rounded-lg hover:bg-[#5a9ee5] transition-colors"
                  >
                    Clear All Filters
                  </button>
                </div>
        ...
        <section className="px-6 md:px-12 pb-16">
          <div className="max-w-[900px] mx-auto">
            <h2 className="text-2xl font-light mb-8">
              ...
              <span className="font-semibold text-[#4a90d9]">{service.name}</span>
            </h2>
            <div className="space-y-4">
              {getServiceFAQs(service.slug).map((faq, idx) => (
                <details
                  key={idx}
                  className="group bg-[rgba(15,22,40,0.6)] border border-[rgba(74,144,217,0.12)] rounded-xl overflow-hidden"
                >
                  <summary className="flex items-center justify-between gap-4 px-5 py-4 cursor-pointer list-none text-[14px] sm:text-[15px] font-medium text-[#e8edf5] hover:text-[#4a90d9] transition-colors">
                    {faq.question}
                    <svg
                      className="w-4 h-4 text-[#6b7a94] flex-shrink-0 transition-transform group-open:rotate-180"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                      strokeWidth={2}
                  ...
                  <div className="px-5 pb-4 text-[13px] text-[#6b7a94] leading-relaxed border-t border-[rgba(74,144,217,0.06)] pt-3">
                    {faq.answer}
                  </div>
                </details>
              ))}
