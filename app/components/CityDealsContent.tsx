...
interface CityDealsContentProps {
  city: DealCity;
  deals: Deal[];
  services: DealServiceCategory[];
}

export default function CityDealsContent({ city, deals, services }: CityDealsContentProps) {
  ...
  const handleEmailSignup = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("https://formspree.io/f/xjggywyr", {
        ...
        body: JSON.stringify({ email: emailInput, _subject: `Deal Alerts - ${city.city}` }),
      });
  ...
  // Service categories with deal counts for this city
  const serviceCards = services
    .map((s) => ({
      ...s,
      count: deals.filter((d) => d.categories.includes(s.filter)).length,
    }))
    .filter((s) => s.count > 0);
    ...
    <div className="min-h-screen bg-[#0a0f1a] text-[#e8edf5]">
      ...
      <section className="pt-32 pb-6 px-6 md:px-12">
        <div className="max-w-[1200px] mx-auto">
          ...
          <h1 className="text-[clamp(28px,4vw,48px)] font-extralight tracking-tight mb-3">
            Car Service Deals in{" "}
            <span className="italic text-[#4a90d9]">{city.city}</span>
          </h1>
          <p className="text-[15px] text-[#6b7a94] max-w-[600px] mb-6">
            Compare {deals.length} {deals.length === 1 ? "deal" : "deals"} from local auto shops in {city.city}, {city.state}. Oil changes, brakes, detailing, and more.
          </p>
        </div>
      </section>
      ...
      {serviceCards.length > 0 && (
        <section className="px-6 md:px-12 pb-8">
          <div className="max-w-[1200px] mx-auto">
            <h2 className="text-[16px] font-medium text-[#e8edf5] mb-4">Browse by Service</h2>
        ...
        <div className="max-w-[1200px] mx-auto">
          ...
          {deals.length === 0 && (
            <div className="text-center py-16">
              <p className="text-[14px] text-[#6b7a94]">No deals found in {city.city} yet.</p>
              <Link href="/deals" className="mt-3 inline-block text-[13px] text-[#4a90d9] hover:text-[#5a9ee5] transition-colors">
                View all deals
              </Link>
            </div>
          )}
      ...
      <section className="px-6 md:px-12 pb-12">
        <div className="max-w-[700px] mx-auto">
          <div className="rounded-2xl border border-[rgba(16,185,129,0.15)] bg-[rgba(16,185,129,0.04)] p-8 text-center">
            <h2 className="text-[20px] font-semibold text-[#e8edf5] mb-2">Get {city.city} Deal Alerts</h2>
            <p className="text-[13px] text-[#6b7a94] mb-5 max-w-[400px] mx-auto">
              New deals added weekly. Get notified about savings in {city.city}.
            </p>
            {emailStatus === "success" ? (
