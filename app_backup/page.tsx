    ...
    <div className="min-h-screen bg-[#0a0f1a] text-[#e8edf5]">
      ...
      <section className="h-screen relative overflow-hidden">
        {heroSlides.map((slide, index) => (
          ...
          >
            <div className="absolute inset-0 bg-cover bg-center opacity-35 blur-[1px]" style={{ backgroundImage: `url(${slide.image})` }} />
            <div className="absolute inset-0 bg-gradient-to-b from-[#0a0f1a]/70 to-[#0a0f1a]/95" />
            <div className={`relative h-full flex flex-col items-center justify-center text-center max-w-[800px] mx-auto px-8 transition-all duration-1000 ${isLoaded && currentSlide === index ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
              <div className="text-[11px] tracking-[0.35em] uppercase text-[#4a90d9] mb-5 font-medium">{slide.category}</div>
              <h1 className="text-[clamp(44px,7vw,88px)] font-extralight leading-[1.1] mb-7 tracking-tight">
                {slide.title}<br />
                <span className="font-semibold bg-gradient-to-r from-[#e8edf5] to-[#4a90d9] bg-clip-text text-transparent">{slide.titleAccent}</span>
              </h1>
              <p className="text-[17px] text-[#6b7a94] leading-relaxed max-w-[550px] mb-10 font-light">{slide.subtitle}</p>
              <div className="flex gap-4">
                <button className="px-9 py-4 text-xs tracking-[0.15em] uppercase bg-[#4a90d9] text-[#0a0f1a] font-semibold hover:bg-[#6ba8eb] transition-all duration-300 hover:-translate-y-0.5 hover:shadow-[0_16px_32px_-8px_rgba(74,144,217,0.3)]">Explore Directory</button>
                <button className="px-9 py-4 text-xs tracking-[0.15em] uppercase bg-transparent text-[#e8edf5] border border-[rgba(74,144,217,0.15)] hover:border-[#4a90d9] hover:text-[#4a90d9] transition-all duration-300">Find Services</button>
              </div>
            </div>
          </div>
        ))}
        <div className="absolute bottom-24 left-1/2 -translate-x-1/2 flex gap-2 z-10">
          {heroSlides.map((_, index) => (
            <button key={index} onClick={() => goToSlide(index)} className={`h-2 rounded transition-all duration-300 ${currentSlide === index ? "w-6 bg-[#4a90d9]" : "w-2 bg-[#3d4a61] hover:bg-[#6b7a94]"}`} />
          ))}
        </div>
        <button onClick={prevSlide} className="absolute left-6 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-[rgba(74,144,217,0.1)] border border-[rgba(74,144,217,0.15)] text-[#e8edf5] text-xl flex items-center justify-center hover:bg-[rgba(74,144,217,0.2)] hover:border-[#4a90d9] transition-all duration-300 z-10">‹</button>
        <button onClick={nextSlide} className="absolute right-6 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-[rgba(74,144,217,0.1)] border border-[rgba(74,144,217,0.15)] text-[#e8edf5] text-xl flex items-center justify-center hover:bg-[rgba(74,144,217,0.2)] hover:border-[#4a90d9] transition-all duration-300 z-10">›</button>
      </section>

      {/* Stats */}
      <section className="grid grid-cols-4 py-16 px-12 bg-[rgba(74,144,217,0.04)] border-y border-[rgba(74,144,217,0.12)]">
        {[
          { value: "200+", label: "Vehicles Covered" },
          { value: "150+", label: "Verified Services" },
          { value: "12", label: "Markets" },
          { value: "50K+", label: "Monthly Readers" },
        ].map((stat, index) => (
          <div key={index} className="text-center">
            <div className="text-[42px] font-extralight text-[#4a90d9] mb-2">{stat.value}</div>
            <div className="text-[10px] tracking-[0.2em] uppercase text-[#6b7a94]">{stat.label}</div>
          </div>
        ))}
      </section>
      ...
      <section className="grid grid-cols-4 gap-2 p-2">
        {categoryCards.map((card, index) => (
          <div key={index} className="h-80 relative overflow-hidden cursor-pointer group rounded">
            ...
            <div className="absolute inset-0 bg-gradient-to-b from-[#0a0f1a]/20 to-[#0a0f1a]/85" />
            <div className="absolute bottom-6 left-6 right-6">
              <div className="text-[10px] tracking-[0.15em] uppercase text-[#4a90d9] mb-1.5">{card.subtitle}</div>
      ...
      <section className="py-24 px-12 bg-[#0a0f1a]">
        <div className="text-center mb-16">
          ...
          <h2 className="text-[clamp(32px,4vw,48px)] font-extralight tracking-tight mb-4">Luxury & Electric Vehicles</h2>
          <p className="text-[15px] text-[#6b7a94] max-w-[500px] mx-auto leading-relaxed">In-depth guides to the most compelling EVs and luxury vehicles. Tesla, Rivian, Lucid, Porsche, and more.</p>
        </div>
          ...
          {featuredCars.map((car) => (
            <div key={car.id} className="bg-gradient-to-b from-[rgba(15,22,40,1)] to-[rgba(10,15,26,1)] rounded overflow-hidden cursor-pointer border border-[rgba(74,144,217,0.15)] hover:border-[rgba(74,144,217,0.4)] transition-all duration-400 hover:-translate-y-1.5 hover:shadow-[0_32px_64px_-16px_rgba(0,0,0,0.5)] group">
              ...
              <div className="p-6">
                ...
                <div className="text-xl font-medium mb-1.5">{car.make} {car.model}</div>
                <div className="text-[13px] text-[#6b7a94] mb-4">{car.year}</div>
      ...
      <section className="py-24 px-12 bg-[#0d1424]">
        <div className="text-center mb-16">
          ...
          <p className="text-[15px] text-[#6b7a94] max-w-[500px] mx-auto leading-relaxed">Verified specialists in detailing, ceramic coating, PPF, and interior restoration.</p>
        </div>
      ...
      <section className="py-24 px-12 bg-[#0d1424]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-7 max-w-[1300px] mx-auto">
          {[
            { title: "US EVs vs Chinese EVs", category: "Comparison", description: "A deep dive into how American and Chinese electric vehicles stack up in range, tech, and value.", image: "https://images.unsplash.com/photo-1593941707882-a5bba14938c7?w=800&q=80" },
            { title: "Best Ceramic Coatings 2024", category: "Guide", description: "We tested the top ceramic coating brands to find the best protection for your premium vehicle.", image: "https://images.unsplash.com/photo-1607860108855-64acf2078ed9?w=800&q=80" },
            { title: "EV Maintenance Guide", category: "Guide", description: "Everything you need to know about maintaining your electric vehicle for peak performance.", image: "https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=800&q=80" },
          ].map((article, index) => (
            <div key={index} className="bg-gradient-to-b from-[rgba(15,22,40,1)] to-[rgba(10,15,26,1)] rounded overflow-hidden cursor-pointer border border-[rgba(74,144,217,0.15)] hover:border-[rgba(74,144,217,0.4)] transition-all duration-400 hover:-translate-y-1.5 hover:shadow-[0_32px_64px_-16px_rgba(0,0,0,0.5)] group">
              <div className="h-44 overflow-hidden">
      ...
      <footer className="bg-[#0a0f1a] pt-16 pb-8 px-12 border-t border-[rgba(74,144,217,0.15)]">
        <div className="grid grid-cols-5 gap-12 max-w-[1200px] mx-auto mb-12">
          {[
